...
namespace DevPratica.Locacao.Controllers
{
    public class FornecedorController : Controller
    {
        ...
        public FornecedorController(IOptions<DadosBase> dadosBase, IApiToken apiToken, IHttpClientFactory httpClient)
        {
        ...
        public async Task<IActionResult> Index(string mensagem = null, bool sucesso = true)
        {
            ...
            if (response.IsSuccessStatusCode)
                return View(JsonConvert.DeserializeObject<List<Fornecedor>>(await response.Content.ReadAsStringAsync()));
            else
        ...
        {
            _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", await _apiToken.Obter());
            HttpResponseMessage response = await _httpClient.GetAsync($"{_dadosBase.Value.API_URL_BASE}Fornecedor/ObterPorCNPJ?cnpj={id}");

            if (response.IsSuccessStatusCode)
                return View(JsonConvert.DeserializeObject<Fornecedor>(await response.Content.ReadAsStringAsync()));
            else
                throw new Exception(response.ReasonPhrase);
        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit([FromForm] Fornecedor fornecedor)
        {
            try
            {
                if (ModelState.IsValid)
